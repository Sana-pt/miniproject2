...
def payment_statuss(request,id):
    data=Bookings.objects.get(id=id)
...
def profile(request):
    ...
    return render(request,'profile.html',{'data':val})
...
def booking(request):
    ...
    if request.method == 'POST':
        ...
        if food_id and decoration_id :
            obj=Bookings.objects.create(event_date=eventdate,user_id=data,hall_id=h,photography=photography,
                                        food_value=food,food=f,no_of_people=no_of_people,decoration_value=decoration
                                        ,decoration=d,photography_cost=photography_amount,total_payment=t_amount)
            obj.save()
        else:
            obj=Bookings.objects.create(event_date=eventdate,user_id=data,hall_id=h,photography=photography,food_value=food,
                                        no_of_people=no_of_people,decoration_value=decoration,photography_cost=photography_amount,
...
def viewhall(request):
   ...
   return render(request,'viewhall.html',{'data':data})
...
def addhall(request):
   if request.method == "POST":
      ...
      hall_description=request.POST.get('description')
